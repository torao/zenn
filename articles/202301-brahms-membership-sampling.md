...
標的型攻撃はある 1 つの正常なノードに大量の攻撃ノードの `push_request` を集中させそのビュー $\mathcal{V}$ を乗っ取ろうとします。しかし Brahms のアルゴリズムでは、多数の `push_request` でビュー $\mathcal{V}$ をポイズニングされないように、1 ラウンドあたりで受信した `push_request` の数がその期待値 $\alpham$ を超えていたらビュー $\mathcal{V}$ を更新しないという動作をします。
...
**プライベートネットワークへの適用**: ノード数が概ね $n$ のビザンチン故障を想定しない小～中規模ネットワークであっても、$n=m=n$ とした Brahms を適用すればノードの動的な参加と離脱をサポートする"ほぼ"全結合のネットワークを構成することができるんじゃないかな。
