...
全体のスケッチとしては、Gossipping モジュールがネットワークに流通しているノード ID を収集し、発見したノード ID で継続的に Sampler ベクトルを更新していると、Sampler ベクトルの状態が「ネットワークからランダムに選んだノード ID の集合」とほぼ同じものになってゆく、というものです。もしノードの参加/離脱がない安定したネットワークで、すべてのノードの ID がネットワークに浸透し切ると、すべてのノードの Sampler ベクトルはそれぞれがネットワークから正しくランダムサンプリングされたノード ID の集合となります (ノードごとにサンプル集合は異なります)。
