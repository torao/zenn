...
さらに、AltaVista Search が実際に採用していた置換族 $\mathcal{F}$ は線形変換 $\pi(x)=ax+b \bmod p$ だったと書いている。これは… ( ﾟдﾟ)ﾊｯ! 線形合同法? つまり線形合同法の疑似乱数生成器を使って一様乱数に変換していたという意味だろう。もちろんこの線形変換では近似 min-wise 独立すらも満たさないのだが、ハッシュ演算や巨大な変換表より効率が良いのは見ての通りだし、これでも実際に当時の実用には耐え得る精度が出たならこまけぇこたぁいいんだよ!! (直感的に $\pi$ が疑似乱数生成器でも十分機能するとは思うが、論文にはこの形式の線形変換を適用したときの確率も記載されている)。
