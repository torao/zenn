...
さらに、AltaVista Search が実際に採用していた置換族 $\mathcal{F}$ は単なる線形変換 $\pi(x)=ax+b \bmod p$ だったようだ。てマジ? 😳 極まってる… (まあハイエンドサーバのメモリが 512MB という御時世だったので)。この線形変換はもちろん近似 min-wise 独立にすらなっていないのだが、ハッシュ演算や巨大な変換表よりパフォーマンスが良いのは見ての通りだし、実際に同時の実用には耐える程度の精度は出ていたのだろう (さすがに採用するために必要だったのか論文には線形変換を適用したときの確率も記載されている)。
