...
現代的な大規模分散システムやマイクロサービスアーキテクチャでは、複数のノードが非同期に連携しながら複雑に協調動作するケースが増えています。そのような環境下ではネットワーク品質のわずかな変動がシステム全体に大きな影響を及ぼすことがあり、大規模システムの開発や運用においてシステムが想定する通信品質を再現したテスト環境を構築することが重要です。
...
ここからは `alpha-ub01` (Ubuntu 24.04) と `beta-ce-01` (CentOS Stream 9) の説明を併記します。まずそれぞれの環境でテストに必要なツールをインストールします。
