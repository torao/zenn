...
# Linux ネットワーク不安定性シミュレーション入門

## 1. はじめに

現代的な大規模分散システムやマイクロサービスアーキテクチャでは、複数のノードが非同期に連携しながら複雑に協調動作するケースが増えています。このような環境下ではネットワーク品質のわずかな変動がシステム全体に大きな影響を及ぼすことがあり、大規模システムの開発や運用においてシステムが想定する通信品質を再現したテスト環境を構築することが重要です。一般的に、完全に接続できないようなわりやすいネットワーク障害よりも、著しい遅延や確率的な瞬断が発生しながら駆動し続けるネットワーク不安定性の方が、システム全体としての事態をより深刻化させる傾向があります。

特にネットワーク不安定性 (遅延や瞬断など) が発生した場合、非同期システムでは以下のような挙動や問題が顕在化する可能性があります。
...
この記事では Linux ネイティブのネットワーク制御ツールである **tc** (+ netem モジュール) と **iptables** を使い、システムテスト環境でネットワーク遅延や通信中の切断などの不安定性を再現する実践的な手順をハンズオン形式で解説します。

## 2. 前提と準備

**対象**: TCP/IP の基礎知識を持ち、Linux のネイティブツールに慣れているエンジニア。

**重要な注意点**: ここで解説している方法はしばしばアプリケーションに「不可解な挙動」を引き起こす可能性があります。これらの挙動を十分に理解し、説明できるスキルがなければ、この記事で得られた結果を適切に活用することは困難かもしれません。また、設定ミスや解除漏れによってシステムが意図しない状態に陥り、最悪の場合、元に戻せなくなるリスクを伴います。操作は自身のスキルレベルを十分に考慮して隔離された環境で慎重に行ってください。
...
ハンズオン形式で進めるために上記の環境に通信可能な 2 つのノードを準備してください。この記事では、以下の代表的な 2 種類の Linux ディストリビューションでの操作を説明するために 2 つのノードに異なる Linux 環境を導入しています。自分の環境に合わせて該当するディストリビューションの説明を参照してください。
...
この状態で `iperf3` を使って正常状態での TCP ベースの帯域速度を測定してみましょう。以下の例では `alpha-ub01` でサーバを起動し `beta-ce01` から接続しています。速度がネットワーク構成で想定するスペックに近ければ OK です (例えばギガビットイーサネット環境であれば 950Mbps 程度)。
...
`tc filter` を使用すると、特定の IP アドレス宛のパケットに対してのみネットワーク障害を注入することができます。これはより現実的で複雑なネットワーク環境のシミュレーションを可能にします。

- **ネットワーク遅延の差異**: ローカルネットワーク内のノードと、地理的に離れた場所に存在するノードとの間の遅延差を再現する。
- **外部サービスとの通信**: 外部の API サービスやデータベースとの通信における不安定性をシミュレーションする。
- **段階的な影響調査**: ネットワーク不安定性の注入範囲を部分的ナノードに対して開始し、徐々に拡大してゆくことで、システム全体への影響を段階的に調査する。
